...
water_bodies <- st_read ("2.Datos/gis_data/ury_inlandwater") %>%  # Uruguay/InlandWater
...
mapa_listo <-  base_map +
  ...
  annotation_north_arrow(location = "bl", which_north = "true", 
                         pad_x = unit(2.2, "in"), pad_y = unit(2.5, "in"),
                         style = north_arrow_fancy_orienteering) +
  labs (x = "longitud", y = "latitud") + 
  geom_sf_text(data = river_labels,
                     aes(label = name), 
                     size = 3, 
                     fontface = "bold",
                     family = "serif",
...
caption_sites <- paste(sites_longlat$site_number,
                       sites_longlat$codigo_estacion, sep = " = ") %>%
  str_c(., collapse = ", ")
