...
data_articulo <- data_articulo %>% filter (nombre_programa != ("Rio Cuareim")) %>% 
    mutate (nombre_programa = fct_recode(nombre_programa, 
                                         RN = "Río Negro",
                                         RU = "Rio Uruguay"),
...
Data_out <- ggplot(data_articulo,aes(y=Clorofila_a, x=nombre_programa)) + 
...
ph_rn_model <- lme4::lmer(Ph ~ Year  + (1 | codigo_pto/Mes), data = ph_rn )
...
Alc_rn_model <- lme4::lmer(Alcalinidad ~ Year  + (1 | codigo_pto/Mes), data = Alc_rn )
